...
var burn_fulling_fueled_accel = 1.26; //Burning Itch can accel at 1.26 m/s/s when unloaded but fully fueled
var burn_burn_time = 350; //Burning Itch can burn for 350 hours
...
document.write("<h3>approx travel time and resource consumption for a continuous burn between these two destinations</h3>");
document.write("<p>assume craft does continuous thrust (prograde for first half, retrograde for second half). This is time it takes to cover linear distance that is minimum distance between these two bodies. Assume it's not actually affected by gravity. (Like I said, \"naive\"). Of course real travel time will take longer. (<strong>\"Burning Itch\" time.</strong>)</p>");
...
for (var m = 1; m < distances.length; m++){
	...
	for (var n = 1; n < distances.length; n++){
		...
		if (m == n){
			...
			time_int = (quadratic(-0.5*burn_fulling_fueled_accel, 0, 0.5*((Math.abs(distances[m]-distances[n]))*1000.0)) * 2.0 / (60.0*60.0));
			hour_string = Math.round(time_int);
			//minute_string = Math.floor((time_int % 1)*60.0);
			time_string += "~" + hour_string + " h, ";
			fuel_cost = 20000 * (time_int/burn_burn_time);
			favor_level = "ERROR";
			if (fuel_cost < 100)
			{
				favor_level = "Trivial";
			} else if (fuel_cost < 500)
			{
				favor_level = "Low";
			} else if (fuel_cost < 1500)
			{
				favor_level = "Medium";
			} else if (fuel_cost < 10000)
			{
				favor_level = "High";
			} else
			{
				favor_level = "Expensive";
			}
			resource_string += favor_level + " favor to refuel";
		}
		document.write("<td>" + time_string + resource_string + "</td>");
	}
	document.write("</tr>");
} 
